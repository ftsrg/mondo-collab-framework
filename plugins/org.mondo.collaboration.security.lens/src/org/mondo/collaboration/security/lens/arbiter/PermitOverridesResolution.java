...
public class PermitOverridesResolution implements SecurityArbiter.RuleConflictResolver {
	...
	private void processReferenceAsset(OperationKind op, Role role,
			Map<Asset, TreeSet<SecurityRuleJudgement>> roleJudgements, ReferenceAsset referenceAsset,
			SecurityRuleJudgement objectJudgement) {
		RuleType referenceRuleType = objectJudgement.getRule().getContraint().getType();
		RuleRights referenceRuleRight = objectJudgement.getRule().getContraint().getRights();

		Set<Asset> assets = roleJudgements.keySet();
		for (Asset asset : assets) {
			if (asset instanceof ObjectAsset) {
				ObjectAsset objectAsset = (ObjectAsset) asset;
				if (referenceAsset.getSource() == objectAsset.getObject()
						|| referenceAsset.getTarget() == objectAsset.getObject()) {
					...
					if (objectRuleType == RuleType.DENY && referenceRuleType == RuleType.PERMIT) {
						SecurityRuleJudgement newJudgement = createDummyJudgement(objectAsset, null, referenceRuleRight,
								RuleType.PERMIT, role);
	...
	private void processAttributeAsset(OperationKind op, Role role,
			Map<Asset, TreeSet<SecurityRuleJudgement>> roleJudgements, AttributeAsset attributeAsset,
			SecurityRuleJudgement objectJudgement) {
		RuleType attributeRuleType = objectJudgement.getRule().getContraint().getType();
		RuleRights attributeRuleRight = objectJudgement.getRule().getContraint().getRights();
		Set<Asset> assets = roleJudgements.keySet();
		for (Asset asset : assets) {
			if (asset instanceof ObjectAsset) {
				ObjectAsset objectAsset = (ObjectAsset) asset;
				if (attributeAsset.getSource() == objectAsset.getObject()) {
					...
					if (objectRuleType == RuleType.DENY && attributeRuleType == RuleType.PERMIT) {
						SecurityRuleJudgement newJudgement = createDummyJudgement(objectAsset, null, attributeRuleRight,
								RuleType.PERMIT, role);
						arbiter.updateJudgement(op, role, objectAsset, newJudgement, true);
	...
	private void processObjectAsset(OperationKind op, Role role,
			Map<Asset, TreeSet<SecurityRuleJudgement>> roleJudgements, ObjectAsset objectAsset,
			SecurityRuleJudgement objectJudgement) {
		RuleType objectType = objectJudgement.getRule().getContraint().getType();
		RuleRights objectRuleRight = objectJudgement.getRule().getContraint().getRights();
			...
			if (asset instanceof AttributeAsset) {
				...
				if (attributeAsset.getSource() == objectAsset.getObject()) {
					...
					if (attributeRuleType == RuleType.PERMIT && objectType == RuleType.DENY) {
						SecurityRuleJudgement newJudgement = createDummyJudgement(objectAsset, null, objectRuleRight,
								RuleType.PERMIT, role);
						arbiter.updateJudgement(op, role, objectAsset, newJudgement, true);
	...
	private SecurityRuleJudgement createDummyJudgement(Asset asset, IPatternMatch justification, RuleRights right,
			RuleType type, Role role) {
