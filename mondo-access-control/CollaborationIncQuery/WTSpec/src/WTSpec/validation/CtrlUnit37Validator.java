/**
 *
 * $Id$
 */
package WTSpec.validation;

import WTSpec.WTCFault;
import WTSpec.WTCInput;
import WTSpec.WTCOutput;
import WTSpec.WTCTimer;

/**
 * A sample validator interface for {@link WTSpec.CtrlUnit37}.
 * This doesn't really do anything, and it's not a real EMF artifact.
 * It was generated by the org.eclipse.emf.examples.generator.validator plug-in to illustrate how EMF's code generator can be extended.
 * This can be disabled with -vmargs -Dorg.eclipse.emf.examples.generator.validator=false.
 */
public interface CtrlUnit37Validator {
	boolean validate();

	boolean validateInput__iInput(WTCInput value);
	boolean validateInput__iBadInputQuality(WTCInput value);
	boolean validateInput__iDontCheckInput(WTCInput value);
	boolean validateOutput__oSendReset(WTCOutput value);
	boolean validateFault__fFault(WTCFault value);
	boolean validateTimer__tCheckInputDelay(WTCTimer value);
	boolean validateBhvParam__bpLogicType(String value);
}
